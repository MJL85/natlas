		...
		for vlan_n, vlan_v in vlan_row:
			...
			for cdp_row in cdp_vbtbl:
				for cdp_n, cdp_v in cdp_row:
					...
					rname = shorten_host_name(cdp_v.prettyPrint(), config.host_domains)
...
def main(argv):
	...
	print('MNet-TraceMAC (mnet suite v%s)' % __version__)
	...
	try:
		...
		print('usage: mnet-tracemac.py -r <root IP> -m <MAC Address> [-c <config file>]')
		sys.exit(1)
	for opt, arg in opts:
		if (opt == '-r'):
			opt_root_ip = arg
